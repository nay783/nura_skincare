...
export default function AdminEditBlogPostPage() {
  ...
  useEffect(() => {
    async function loadPostDetails() {
      ...
      if (error) {
        setErrorMsg("Erro ao carregar artigo: " + error.message);
      } else if (!post) {
        setErrorMsg("Artigo não encontrado.");
      } else {
        setPostData(post);
        setTitle(post.title);
        setSlug(post.slug);
        setExcerpt(post.excerpt);
        setReadTime(String(post.read_time));
        setKeywordsInput(Array.isArray(post.keywords) ? post.keywords.join(", ") : "");
        setFeaturedImage(post.featured_image || "");
        setHtmlContent(post.html_content || "");
  ...
  const handleUpdatePost = async (e: React.FormEvent) => {
    ...
    if (!excerpt.trim()) {
      setErrorMsg("O resumo é obrigatório.");
      return;
    }
    if (!htmlContent.trim()) {
      setErrorMsg("O corpo do artigo está vazio.");
      return;
    }
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          rich_content: { html: htmlContent.trim() },
          status,
        })
        .eq("id", id);
      ...
      console.error(err);
      setErrorMsg(err.message || "Erro ao guardar alterações.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (!postData && !loading) {
    return (
      <div className="p-8 text-center text-xs text-muted-foreground italic">
        {errorMsg || "Artigo não encontrado."}
        <div className="mt-4">
  ...
  return (
    <div className="space-y-6 font-sans pb-12">
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-2xl font-serif text-primary font-medium">Editar Artigo</h1>
          <p className="text-xs text-muted-foreground">Ficha de edição: {postData.title}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleUpdatePost} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-border p-6 rounded-[4px] space-y-4 shadow-sm">
            ...
            <div className="pt-2">
              ...
              >
                {submitting ? "A guardar..." : "Gravar Alterações"}
              </Button>
            </div>
          </div>
